...
def blocks(path):
    return {int(match[2]): (int(match[1]), match[3]) for match in re.finditer(r"^--- !u!(\d+) &(\d+)(?: stripped)?\n(.*?)(?=^--- !u!|\Z)", read(path), re.M | re.S)}

def canonical(body):
    # Unity's scene save moves the root serialization version past object metadata.
    # Preserve its exact value while normalizing only that field's position.
    lines = re.sub(r"(?<=: )-0(?=[,}])", "0", body.strip()).splitlines()
    versions = [line for line in lines if line.startswith("  serializedVersion:")]
    return "\n".join(versions + [line for line in lines if not line.startswith("  serializedVersion:")])

def equal(name, before, after):
    before, after = canonical(before), canonical(after)
...
local_ids = {5: 806453279, 11: 806453282, 23: 1807628492, 24: 806453281, 32: 806453280}
def remap_local(body):
    return re.sub(r"\{fileID: (\d+)\}", lambda match: "{fileID: "+str(local_ids.get(int(match[1]), int(match[1])+4000000000 if int(match[1]) else 0))+"}", body)
...
checks.append(equal("UI camera ancestor including source child order", remap_local(source[12][1]), active[4000000012][1]))
for source_id in (5, 11):
    checks.append(equal(f"EventSystem hierarchy record {source_id}", remap_local(source[source_id][1]), active[local_ids[source_id]][1]))
assert active[local_ids[23]][0] == 224
assert "m_PrefabInstance: {fileID: 1807628491}" in active[local_ids[23]][1]
assert "m_TransformParent: {fileID: 4000000012}" in active[1807628491][1]

report = {"inputs": [{"path": path.relative_to(root).as_posix(), "sha256": hashlib.sha256(path.read_bytes()).hexdigest()} for path in (source_scene, active_scene, source_light, active_light)], "cameraScriptRemap": {"sourceGuid": source_script_guid, "officialPackageGuid": active_script_guid}, "additionalLocalIdMap": local_ids, "checks": checks}
