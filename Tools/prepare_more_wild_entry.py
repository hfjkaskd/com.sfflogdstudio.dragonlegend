"""Preserve original More Wild icon mesh influences for the Unity rig author."""
import json
from pathlib import Path

root = Path(__file__).resolve().parents[1]
data = json.loads((root / 'Tools/Evidence/ef_morewildicon.json').read_text(encoding='utf8'))
meshes = []
for attachment in data['attachments']:
    ...
    meshes.append(mesh)
output = root / 'Artifacts/MoreWildAuthoring'
output.mkdir(parents=True, exist_ok=True)
(output / 'ef_morewildicon.json').write_text(json.dumps({'meshes': meshes}), encoding='utf8')
