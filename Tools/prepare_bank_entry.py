"""Preserve original Bank icon mesh influences for the Unity rig author."""
import json
from pathlib import Path

root = Path(__file__).resolve().parents[1]
data = json.loads((root / 'Tools/Evidence/ef_slyinhang.json').read_text(encoding='utf8'))
meshes = []
for attachment in data['attachments']:
    ...
    meshes.append(mesh)
output = root / 'Artifacts/BankEntryAuthoring'
output.mkdir(parents=True, exist_ok=True)
(output / 'ef_slyinhang.json').write_text(json.dumps({'meshes': meshes}), encoding='utf8')
