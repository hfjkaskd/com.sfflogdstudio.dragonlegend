"""Prepare native Free-start meshes and independently sampled original geometry."""
import copy
import json
import struct
...
author = root / 'Artifacts/FreeStartAuthoring'
author.mkdir(parents=True, exist_ok=True)
rigs = []
for name in ('ef_starttc', 'ef_xjpl'):
...
(root / 'Tools/Evidence/free-start-poses.json').write_text(json.dumps({'rigs': rigs}, indent=2), encoding='utf8')
print('Free start:', len(rigs), 'clips,', sum(len(r['frames']) for r in rigs), 'source geometry frames')
