...
Only Unity AnimationCurve, Transform, MonoBehaviour and standard UI components are used. The five-key opening curve represents each OutBounce quadratic segment with exact endpoint derivatives; closing uses the InQuad endpoint derivatives. Per-frame iteration reuses the service list; a sequence enumerator is allocated once when a flip begins. There is no reflection, third-party assembly or runtime static UI construction.
...
RecoveredTreasureCardTests passes both focused PlayMode tests. It checks native hierarchy references and sprite dimensions, 32 actual animation frames against independent InQuad/OutBounce formulas, midpoint visibility, time-scale pause, duplicate Flip suppression, exact sound/event/callback order, and unchanged Y/Z. A second test checks Init during playback, playback while disabled, and no completion callback after destruction.

Full Unity 2022.3.62f3 PlayMode regression also passed **325/325**, `Artifacts/treasure-card-tests.xml`.
...
UITreasureView, main-card flight and its RefreshCollectCard event, live collection tip, claim/ad branches, actual payout and production Free-loop binding remain pending. This prefab is the native card component required by that window, not a finished Treasure mini-game.
