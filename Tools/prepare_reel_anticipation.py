"""Prepare native Reel anticipation meshes and independently sampled original geometry."""
import copy
import json
import struct
...
author = root / 'Artifacts/ReelAnticipationAuthoring'
author.mkdir(parents=True, exist_ok=True)
rigs = []
for name in ('ef_slliejiasu',):
...
(root / 'Tools/Evidence/reel-anticipation-poses.json').write_text(json.dumps({'rigs': rigs}, indent=2), encoding='utf8')
print('Reel anticipation:', len(rigs), 'clips,', sum(len(r['frames']) for r in rigs), 'source geometry frames')
