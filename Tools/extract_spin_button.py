"""Extract this original 4.1.24 region-only button; reject unsupported data, never skip it.
...
for i in range(var()):
 slot=var()
 for j in range(var()):
  key=ref();name=ref();assert u8()==0,'non-region attachment'
  d={'slot':slot,'key':key,'name':name or key,'path':ref(),'values':[num() for _ in range(7)],'color':color()}
  assert u8()==0,'sequence';result['attachments'].append(d)
...
for _ in range(var()):
 ...
 for label in ['ik animation','transform animation','path animation','deform animation','draw order','event animation']:zero(label)
...
regions=[]
for line in Path(sys.argv[1]).with_name('ef_slspineaniu.atlas.txt').read_text().splitlines():
