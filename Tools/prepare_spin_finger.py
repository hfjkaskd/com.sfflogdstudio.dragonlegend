"""Prepare native Spin finger meshes and independently sampled original geometry."""
import copy
import json
import struct
...
author = root / 'Artifacts/SpinFingerAuthoring'
author.mkdir(parents=True, exist_ok=True)
rigs = []
for name in ('ef_shouzhi',):
...
(root / 'Tools/Evidence/spin-finger-poses.json').write_text(json.dumps({'rigs': rigs}, indent=2), encoding='utf8')
print('Spin finger:', len(rigs), 'clips,', sum(len(r['frames']) for r in rigs), 'source geometry frames')
