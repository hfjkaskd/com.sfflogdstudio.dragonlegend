"""Prepare native Free-end meshes and independently sampled original geometry."""
import copy
import json
import struct
...
author = root / 'Artifacts/FreeEndAuthoring'
author.mkdir(parents=True, exist_ok=True)
rigs = []
for name in ('ef_overtc',):
...
(root / 'Tools/Evidence/free-end-poses.json').write_text(json.dumps({'rigs': rigs}, indent=2), encoding='utf8')
print('Free end:', len(rigs), 'clips,', sum(len(r['frames']) for r in rigs), 'source geometry frames')
