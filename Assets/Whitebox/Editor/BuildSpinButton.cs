...
// Offline asset conversion for the verified region-only spin button, not a Spine runtime.
public static class BuildSpinButton
{
    ...
    private const string Folder = "Assets/Resources/RecoveredUI/SpinButton";
    private static readonly string[] Rgba = {"r","g","b","a"};
    public static void Save()
    {
        Directory.CreateDirectory(Folder); AssetDatabase.Refresh();
        var data = JsonUtility.FromJson<Data>(File.ReadAllText("Assets/Whitebox/Editor/RecoveredSpinButton.json"));
        const string texturePath = "Assets/Resources/RecoveredArt/Res/Spine/spin/ef_slspineaniu.png";
        ...
        var root = new GameObject("SpinBtn", typeof(RectTransform), typeof(Button), typeof(Animation), typeof(RecoveredSpinButton));
        try {
            root.layer=5; var rect=(RectTransform)root.transform; rect.sizeDelta=new Vector2(222,219.41f);
            var visual=Rect("Visual",root.transform); visual.anchoredPosition=new Vector2(0,1.7929688f);
            ...
            foreach(var attachment in data.attachments) {
                ...
                image.color=ColorOf(slot.color)*ColorOf(attachment.color);image.raycastTarget=attachment.slot==1;
            ...
            var button=root.GetComponent<Button>();button.targetGraphic=images[1];button.transition=Selectable.Transition.None;
            ...
            foreach(var source in data.animations) {
                var clip=new AnimationClip{name=source.name,legacy=true,frameRate=30,wrapMode=source.name=="idle"?WrapMode.Loop:WrapMode.Once};
            ...
            var serialized=new SerializedObject(root.GetComponent<RecoveredSpinButton>());
            serialized.FindProperty("button").objectReferenceValue=button;serialized.FindProperty("animationPlayer").objectReferenceValue=player;
            serialized.FindProperty("setup").objectReferenceValue=baseline;serialized.ApplyModifiedPropertiesWithoutUndo();
            PrefabUtility.SaveAsPrefabAsset(root,"Assets/Resources/RecoveredUI/SpinButton.prefab");AssetDatabase.SaveAssets();
