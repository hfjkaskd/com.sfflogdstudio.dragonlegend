...
// Only zcjb_idle: setup has no attachments; this clip enables slots 25, 26 and 31.
// Other animations, including meshes/deforms, stay in the extracted evidence JSON.
public static class BuildBonusCoinIdle
{
    ...
    private const string Folder = "Assets/Resources/RecoveredUI/BonusCoinIdle";
    ...
    public static void Save()
    {
        ...
        var idle=Array.Find(data.animations,a=>a.name=="zcjb_idle");
        if(idle==null)throw new InvalidOperationException("Missing original zcjb_idle clip.");
        foreach(var timeline in idle.timelines) {
            ...
            foreach(var frame in timeline.frames)if(frame.attachment!=null) {
                if(timeline.index!=25 && timeline.index!=26 && timeline.index!=31)
        ...
        try {
            ...
            foreach(var attachment in data.attachments) {
                if (attachment.slot != 25 && attachment.slot != 26 && attachment.slot != 31) continue;
            ...
            foreach(var source in data.animations) {
                if (source.name != "zcjb_idle") continue;
                var clip=new AnimationClip{name=source.name,legacy=true,frameRate=30,wrapMode=WrapMode.Loop};
            ...
            PrefabUtility.SaveAsPrefabAsset(root,"Assets/Resources/RecoveredUI/BonusCoinIdle.prefab");AssetDatabase.SaveAssets();
