...
public sealed class RecoveredMainCashOutEntryTests
{
    ...
    public IEnumerator MainButtonOpensCachedCashOutAndGmRemovesOldBinding()
    {
        ...
        try{
            ...
            var topButton=game.BalancePanel.WithdrawButton;
            Assert.IsNotNull(topButton);Assert.AreEqual("WithdrawBtn",topButton.name);
            Assert.AreEqual(0,topButton.onClick.GetPersistentEventCount());
            var topArea=(RectTransform)topButton.transform.parent;
            Assert.AreSame(game.BalancePanel.transform,topArea.parent);
            Assert.AreEqual(new Vector2(430.15f,109.55f),topArea.sizeDelta);
            Assert.AreEqual(new Vector2(5.45f,0),topArea.anchoredPosition);
            var topRect=(RectTransform)topButton.transform;
            Assert.AreEqual(Vector2.zero,topRect.anchorMin);Assert.AreEqual(Vector2.one,topRect.anchorMax);
            Assert.AreEqual(new Vector2(-94.3672f,0),topRect.sizeDelta);
            var art=(RectTransform)topButton.transform.Find("SkeletonGraphic (ef_sltxan)");
            Assert.IsNotNull(art);
            var sourcePosition=topArea.InverseTransformPoint(art.position);
            Assert.AreEqual(-5,sourcePosition.x,.001f);Assert.AreEqual(11-109.55f/2,sourcePosition.y,.001f);
            var topLabel=topButton.GetComponentInChildren<TMPro.TextMeshProUGUI>();
            Assert.AreEqual("CASH OUT",topLabel.text);Assert.AreEqual(39,topLabel.fontSize);
            Click(topButton,camera);var topWindow=core.CashOut;Assert.IsNotNull(topWindow);
            for(int i=0;i<10;i++)yield return null;
            Click(topWindow.BackButton,camera);for(int i=0;i<10;i++)yield return null;
            var topAdapt=game.Playfield.GetComponent<RecoveredScreenAdapt>();
            topAdapt.Apply(1080,1920,new Rect(24,60,1032,1740));
            Click(topButton,camera);Assert.AreSame(topWindow,core.CashOut);
            for(int i=0;i<10;i++)yield return null;
            Click(topWindow.BackButton,camera);for(int i=0;i<10;i++)yield return null;
            topAdapt.AdaptScreen();
            ...
            topButton.onClick.Invoke();Assert.AreEqual(count,sounds.Count,"Old top entry must release its callback too.");
